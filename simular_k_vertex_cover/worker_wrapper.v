...
module worker_wrapper # (
    ...
    output reg  [511:0]  res_mask_core,
    output reg  [511:0]  res_mask_neighbor,
    output reg  [31:0]   res_k_core,
    output reg  [31:0]   res_k_neighbor,
    
    // NOVAS PORTAS PARA GARANTIR QUEM VENCEU
    output reg  [511:0]  res_mask_sol,
    output reg  [31:0]   res_k_sol
);

    reg  [63:0] rom [0:511];
    wire [8:0]  addr_core, addr_neighbor;
    wire [63:0] mem_data_core     = rom[addr_core];
    wire [63:0] mem_data_neighbor = rom[addr_neighbor];

    wire done_c, done_n;
    wire [1:0] result_c, result_n;
    wire [31:0] w_k_core, w_k_neighbor;
    wire [511:0] w_mask_core, w_mask_neighbor;

    reg core_busy, neigh_busy, task_active;

    vertex_cover_core u_core (
    ...
    vertex_remove_neighbor u_neighbor (
        .clk(clk), .rst(rst), .start(start),
    ...
    always @(posedge clk or posedge rst) begin
        if (rst || solution_found_global) begin
            ...
            if (start) begin
                ...
                if (core_busy && done_c) begin
                    ...
                    if (result_c == 2'd1) begin 
                        solution_found <= 1'b1;
                        res_mask_sol <= w_mask_core; // Salva a máscara real da solução!
                        res_k_sol <= w_k_core;       // Salva o K real da solução!
                    end
                    else if (result_c == 2'd0) push_core <= 1'b1; 
                end

                // 2. Captura do NEIGHBOR
                if (neigh_busy && done_n) begin
                    ...
                    if (result_n == 2'd1) begin
                        solution_found <= 1'b1;
                        res_mask_sol <= w_mask_neighbor; // Salva a máscara real da solução!
                        res_k_sol <= w_k_neighbor;       // Salva o K real da solução!
                    end
